...
const FIND_RESTROOM_ACTION = { label: "Find nearest restroom", prompt: "Where is the nearest restroom?" };
...
async function handleRestroomRequest(request: AgentRequest): Promise<AgentResponse> {
  ...
  const toolCalls: AgentResponse["toolCalls"] = [];
  let reply = "I couldn't find a restroom nearby right now — try asking a volunteer close to your section.";
  ...
  if (facilityTool) {
    ...
    toolCalls.push({ toolName: facilityTool.name, input: { type: "restroom" }, output: result.data });
  ...
  return { agentId: "navigation", reply, toolCalls, suggestedActions: [FIND_EXIT_ACTION], requiresClarification: false };
}

async function handleExitRequest(request: AgentRequest): Promise<AgentResponse> {
  const routeTool = getTool("route");
  const toolCalls: AgentResponse["toolCalls"] = [];
  let reply = "I don't have live exit congestion data yet, but any gate you entered through works as an exit.";
  ...
  return { agentId: "navigation", reply, toolCalls, suggestedActions: [FIND_RESTROOM_ACTION], requiresClarification: false };
}
...
async function handle(request: AgentRequest): Promise<AgentResponse> {
  ...
  return {
    agentId: "navigation",
    reply: "I can help you navigate the stadium — tell me if you're looking for your seat, a restroom, or an exit.",
    toolCalls: [],
    suggestedActions: [FIND_RESTROOM_ACTION, FIND_EXIT_ACTION],
    requiresClarification: false,
  };
